...
void cgiHeaderLocation(char *redirectUrl) {
	fprintf(cgiOut, "Location: %s%c%c%c%c", redirectUrl, 13, 10, 13, 10); 
}

void cgiHeaderStatus(int status, char *statusMessage) {
	fprintf(cgiOut, "Status: %d %s%c%c%c%c", status, statusMessage, 
	13, 10, 13, 10);
}

void cgiHeaderContentType(char *mimeType) {
	fprintf(cgiOut, "Content-type: %s%c%c%c%c", mimeType, 13, 10, 13, 10);
}
