...
int StartSQL()
{
	...
	if (mysql_query(&mysql,
		...
		"sleep=0 and god<>2 and fightable<>0"))
	{
...
//		myLog("/karchan/mud/data/error.log", "Number of rows: %i\n", number_of_Names);
		...
		while ((i<number_of_Names) && (!checkbot))
		{
			...
			mysql_free_result(res);
				...
				if (!strcmp(race, "rabbit")) {itemid=43;}
				if (!strcmp(race, "duck")) {itemid=44;}
				if (!strcmp(race, "wolf")) {itemid=45;}
				if (!strcmp(race, "chipmunk")) {itemid=46;}
				if (!strcmp(race, "fox")) {itemid=47;}
				if (!strcmp(race, "ropegnaw")) {itemid=48;}
				if (!strcmp(race, "buggie")) {itemid=49;}
				if (!strcmp(race, "ooze")) {itemid=50;}
				if (!strcmp(race, "troll")) {itemid=51;}
				if (!strcmp(race, "wyvern")) {itemid=52;}
				if (!strcmp(race, "zombie")) {itemid=53;}
				if (!strcmp(race, "sentry")) {itemid=54;}
				sprintf(sqlstring, "update tmp_itemtable set amount=amount+1 "
				"where (id=%i) and "
				"(search='') and "
				"(belongsto='') and "
