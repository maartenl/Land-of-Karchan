...
public class UnwieldCommand extends NormalCommand
{
			...
			MmudException
	{
		...
		for (int i = 0; ((i < myItems.size()) && (j != amount)); i++)
		{
			...
			if (success)
			{
				...
				return stopWielding(myItem, aPlayer, position);
			}
		}
		return false;
	}

	public static boolean stopWielding(Item item, Player aPlayer, PersonPositionEnum position)
		...
		MmudException
	{
	    ...
	    Database.writeLog(aPlayer.getName(), "stops wielding " + item
			    + " " + position);
	    Persons.sendMessage(aPlayer, "%SNAME stop%VERB2 wielding "
