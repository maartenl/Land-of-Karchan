...
public class BuyCommand extends NormalCommand
{
  ...
  /**
   * Tries out the buy command. There are a couple of requirements that need to
   * be met, before a successful sale takes place.
   * <ol>
   ...
   * <li>the item itself should <I>NOT</I> have a attribute called "notbuyable".
  ...
  public DisplayInterface run(String command, User aUser) throws MudException
  {
    ...
    {
      communicationService.writeMessage("The shopkeeper doesn't have that.<br/>\n");
      return aUser.getRoom();
    }
    if (itemsFound.size() < amount)
    {
      communicationService.writeMessage("The shopkeeper doesn't have that many items in stock.<br/>\r\n");
      return aUser.getRoom();
    }
    ...
    for (Item item : itemsFound)
    {
      ...
      if (!item.isBuyable())
      {
        communicationService.writeMessage("You cannot buy that item.<BR>\r\n");
        continue;
      }
      ...
      CommunicationService.getCommunicationService(aUser.getRoom()).sendMessage(aUser, "%SNAME buy%VERB2 " + item.getDescription() + " from " + shopkeeper.getName() + " for " + OutputFormatter.getDescriptionOfMoney(moneyPaid) + ".<br/>\r\n");
      bought = true;
      amount--;
      if (amount == 0)
      {
    ...
    if (!bought)
    {
      communicationService.writeMessage("You did not buy anything.<br/>\r\n");
    } else
    {
      communicationService.writeMessage("You bought some of the items.<br/>\r\n");
    }
    return aUser.getRoom();
  }
