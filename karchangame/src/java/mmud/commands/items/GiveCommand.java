 ...
 * &lt;amount&gt; &lt;item&gt; to &lt;character&gt;". It is also possible
...
public class GiveCommand extends NormalCommand
{
    ...
    private boolean giveMoney(List<String> parsed, int amount, User aUser, Person target) throws MudException
    {
        ...
        aUser.getRoom().sendMessage(aUser, target, "%SNAME give%VERB2 "
    ...
    private void giveItems(User aUser, List<String> parsed, int amount, Person target) throws MudException
    {
        ...
        for (Item item : itemsFound)
        {
            if (aUser.unused(item) && !item.isBound())
            {
                ...
                aUser.getRoom().sendMessage(aUser, target, "%SNAME give%VERB2 " + item.getDescription() + " to %TNAME.<br/>\r\n");
                given = true;
                amount--;
                if (amount == 0)
                {
        ...
        if (!given)
        {
            aUser.writeMessage("You did not give anything.<br/>");
        } else
        {
            aUser.writeMessage("You gave some of the items.<br/>\r\n");
        }
    }
}