...
function writeStuff(data)
{
  ...
  {
    body += "<p>[";
    if (data.west !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goWest();'>west</a> ";
    }
    if (data.east !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goEast();'>east</a> ";
    }
    if (data.north !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goNorth();'>north</a> ";
    }
    if (data.south !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goSouth();'>south</a> ";
    }
    if (data.up !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goUp();'>up</a> ";
    }
    if (data.down !== undefined) {
      body += "<a href=\"javascript:void(0)\" onclick='goDown();'>down</a> ";
    }
    body += "]</p>";
  }
  if (data.persons !== undefined)
  {
    if (data.persons.lenght !== 0) {
      body += "<p>";
    }
...
{
  if (window.console)
    console.log("toggleSleep sleep was " + Karchan.sleep);
  if (Karchan.sleep)
  {
    // AWAKEN!!!
    if (window.console)
...
function clearLog()
{
  ...
  Karchan.logSize = 0;
  if (window.console)
    console.log("clearLog setting offset to " + Karchan.logOffset);
  $("#karchan_log").html("");
  $("#warning").html("");
  $('#command').focus();
}
...
function goWest() {
  processCall("go west", false, writeStuff);
  return false;
}
function goEast() {
  processCall("go east", false, writeStuff);
  return false;
}
function goNorth() {
  processCall("go north", false, writeStuff);
  return false;
}
function goSouth() {
  processCall("go south", false, writeStuff);
  return false;
}
function goUp() {
  processCall("go up", false, writeStuff);
  return false;
}
function goDown() {
  processCall("go down", false, writeStuff);
  return false;
}
