/*-------------------------------------------------------------------------
cvsinfo: $Header: /karchan/mud/cvsroot/server/mmud/commands/LockCommand.java,v 1.3 2005/09/15 04:50:00 karn Exp $
...
public class LockCommand extends NormalCommand
{
	...
	throws ItemException, ParseException, MudException
	{
		...
		if (myParsed.length > 1)
		{
			...
			if (myCon.isOpen())
			{
				aUser.writeMessage(aContainer.getDescription() + " is open. Close it first.<BR>\r\n");
				return true;
			}
			if (myCon.isLocked())
			{
				aUser.writeMessage(aContainer.getDescription() + " is already locked.<BR>\r\n");
				return true;
			}
			...
			myCon.setLidsNLocks(myCon.isOpenable(),
				...
				myCon.getKeyId(),
				true);
			Persons.sendMessage(aUser, "%SNAME lock%VERB2 " + aContainer.getDescription() + 
